...
function renderProgresoGeneral(){const totalItems=programas.length*checks.length;const completos=programas.reduce((acc,p)=>acc+checks.filter(k=>!!p[k]).length,0);const porcentaje=totalItems?Math.round(completos/totalItems*100):0;$('progresoGeneral').innerHTML=`<div class=\"overall-progress-top\"><span>AVANCE TOTAL</span><strong>${porcentaje}%</strong></div><div class=\"overall-progress-bar\"><div style=\"width:${porcentaje}%\"></div></div>`}
...
function renderTodo(){renderProgresoGeneral();renderStats();renderProgramas();renderCalendario()}
